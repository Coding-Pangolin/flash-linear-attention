...
### 实测结果（H20，`model-gk` T=11264 HK=HV=32 K=V=128，event p50）
...
1. **`both ≈ h + m`，不是 `max(h,m)` —— 两半并没有并行。** 128 个 program 铺在 78 个 SM 上
   只有 1.64 波。所以"两条独立链可以并行"这个直觉在 H20 上就不成立。
2. **m 半边是瓶颈**：default 下占 65%，ieee 下占 90%。m 的 MAC 只有 h 的 2.00×，
   耗时却是 2.73×（default）。
3. **h 半边对 precision 几乎不敏感（+3.5%）**，因为它的两个 dot 输入都是 BF16；
   m 半边跨档位差 **41×**，全部来自 `M_c @ m` 那一个 FP32 乘。

注意 `--half m` 通过把 V 置 0 实现，编译器会把 h 分支判成死代码，因此 m 的实测值偏乐观；
...
| 噪声基准A = `|单调用<cp-precision> - 单调用ieee|` | 同一支 kernel、不做任何 merge、只换精度档位时的抖动 |
| 噪声基准B = `|同精度拆块复合 - 同精度单调用|` | 同一支 kernel、**同一精度**，只把前缀按 part 边界拆成多段再前缀复合（= 竞品 merge 的语义），与"一次算完"的差 |
| CP 路径 = `|init - 单调用ieee|` | 竞品 chained-merge 的结果与真值的差 |

判读：**CP 路径 ≈ 噪声基准B** → 语义一致（差异只是"分块口径不同"落在不同 bf16 量化点上）；
差出几个量级 → 不一致。以 B 为主基准的原因是：窗口短的时候 `|tf32x3 - ieee|` 经常正好是 0
（tf32x3 本来就接近 fp32），只靠 A 会判不出东西；而"前缀被 part 边界拆开"才是 CP 路径相对
"一次算完"的本质差别。`aligned` 那一路没有段需要携带状态，只看"期望为 0 的段上 |init|max"。
